...
function Publish-Backend {
    ...
    # Handle shared vendor for file system deployment
    if (-not $FTP -or $FileSystemOnly) {
        $sharedVendorPath = "$DEST_ROOT\vendor"
        
        if (-not $ForceVendor) {
            if (!(Test-Path $sharedVendorPath)) {
                Write-Info "Creating shared vendor directory at $sharedVendorPath"
                Ensure-Directory $sharedVendorPath
                if (Test-Path "$BACKEND_SRC\vendor") {
                    Copy-Item "$BACKEND_SRC\vendor\*" $sharedVendorPath -Recurse -Force
                }
            } else {
                Write-Info "Using existing shared vendor at $sharedVendorPath"
            }
            
            # Create junction from project vendor to shared vendor
            $projectVendorPath = "$BACKEND_DEST\vendor"
            if (Test-Path $projectVendorPath) {
                Remove-Item $projectVendorPath -Recurse -Force
            }
            New-Item -ItemType Junction -Path $projectVendorPath -Target $sharedVendorPath
            Write-Info "Created vendor junction: $projectVendorPath -> $sharedVendorPath"
        }
    }
    ...
    if (-not $FTP -or $FileSystemOnly) {
        ...
        $excludePatterns = @(
            ...
            "var\cache\*", "vendor\*", "*.md", "composer.lock", "phpunit.xml", "*.ps1", "debug*.php",
    ...
    if ($FTP) {
        if (Test-FTPConnection -config $FTPConfig) {
            $excludePatterns = @(
                ...
                "storage/logs/*", "storage/cache/*", "var/cache/*", "vendor/*",
                "*.md", "composer.lock", "phpunit.xml", "*.ps1", "debug*.php", "test*.php", "install.php"
            )
    ...
    # Copy 2014_dnd data directory (needed for database setup)
    if (-not $FTP -or $FileSystemOnly) {
        $dataSrc = "$PSScriptRoot\2014_dnd"
        $dataDest = "$DEST_DIR\2014_dnd"
        if (Test-Path $dataSrc) {
            Write-Info "Copying SRD data directory..."
            if (Test-Path $dataDest) {
                Remove-Item $dataDest -Recurse -Force
            }
            Copy-Item $dataSrc $dataDest -Recurse -Force
            Write-Success "SRD data directory copied to: $dataDest"
        }
    }
...
OPTIONS:
    ...
    -ForceVendor, -fv     Force local vendor directory instead of shared (default: use shared vendor)
...
EXAMPLES:
    ...
    .\publish.ps1 -b -ftp -ForceVendor      # Backend to FTP, force local vendor upload
...
FEATURES:
    - Shared vendor: Uses shared vendor directory at deployment root (use -ForceVendor for local)
